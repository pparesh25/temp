# -*- coding: utf-8 -*-
"""
Created on Sun May 21 11:15:34 2023
...
valid_index_names = {'Nifty 50', 'Nifty Next 50'}
...
# Rename the files and process them
files_renamed = False
for filename in os.listdir(destination_dir):
    if filename.startswith("ind_close_all_") and filename.endswith(".csv"):
        ...
        # Extract the values from the 'Index Date' column and store the first 4 letters in variable 'B'
        B = data_frame['Index Date'].astype(str)

        # Check if A is not equal to B
        if A != B.iloc[0]:
            print("Date format corrected for file:", filename)

            # Update the 'Index Date' column with A
            data_frame['Index Date'] = A

            # Save the updated DataFrame back to the CSV file
            data_frame.to_csv(file_path, index=False)
            
            print("Updated 'Index Date' column with", A)
