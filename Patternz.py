# -*- coding: utf-8 -*-
"""
Created on Wed Jun  7 22:24:52 2023
...
# Convert 'Breakout Date', 'Start', and 'End' columns from DD-MM-YYYY to YYYY-MM-DD format
df['Breakout Date'] = pd.to_datetime(df['Breakout Date'], format='%m/%d/%Y')#.dt.strftime('%Y-%m-%d')
df['Start'] = pd.to_datetime(df['Start'], format='%d-%m-%Y')#.dt.strftime('%Y-%m-%d')
df['End'] = pd.to_datetime(df['End'], format='%d-%m-%Y')#.dt.strftime('%Y-%m-%d')
